...
def draw_main_menu():
    ...
    image_intro = pygame.image.load('start2.png') #PyCar.png 480x800(넓, 높)
    screen.blit(image_intro, [draw_x, draw_y - 500 ]) # -280
...
''' def draw_main_menu():
    draw_x = (WINDOW_WIDTH / 2) - 200
    draw_y = WINDOW_HEIGHT / 2
    image_end = pygame.image.load('start.png') #PyCar.png 480x800(넓, 높)
        ...
        if not crash:
            ...
            for i in range(obstacle_count):
                ...
                if obstacles[i].y > WINDOW_HEIGHT:
                    score += 10
                    obstacles[i].x = random.randrange(0, WINDOW_WIDTH-obstacles[i].width)
                    obstacles[i].y = random.randrange(-150, -50) # -150, -50
